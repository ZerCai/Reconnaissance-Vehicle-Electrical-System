...
Next for motor control, the ESP32 receives commands from an Xbox Controller through Bluetooth to control the speed and direction of six motors driven by three motor drivers using PWM signals. Additionally, in the camera stabilization subsystem, an IMU communicates with another ESP32 to send data on the vehicle's orientation during traversal. This ESP32 uses this data to control the servo motor on the gimbal to stabilize the camera based on the vehicle’s orientation. 
